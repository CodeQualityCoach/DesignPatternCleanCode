...
namespace PdfTools.Commands
{
    ...
    public class ShuffleCommand : ICommand
    {
        ...
        public ShuffleCommand(IDocumentHandlerFactory handlerFactory = null, IFileSystem fileSystem = null)
        {
            _fileSystem = fileSystem ?? new FileSystem();
            _handlerFactory = handlerFactory ?? new PdfHandlerFactory();
        }

        public string Usage { get; } = @"usage: `pdftools shuffle <output> <input 1> <input 2> [input 3..n]`
...
Shuffles two or more input files <input n> into a single pdf <output>";
        ...
        private void DoExecute(string[] args)
        {
            ...
            using (var handler = _handlerFactory.CreateEmpty())
            {
                // let us append all the other files to the first file
                handler.Shuffle(fileNames);
